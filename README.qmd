---
title: "`fanyi`: Translate Words or Sentences via Online Translators"
format: 
  gfm:
...
`r badger::badge_cran_release('fanyi', color='green')`
`r badger::badge_cran_download('fanyi', type='grand-total', color='green')`
`r badger::badge_cran_download('fanyi', type='last-month', color='green')`
`r badger::badge_cran_download('fanyi', type='last-week', color='green')`
...
You should use `set_translate_source()` to set the default translator used in `translate()`.
...
1. go to <https://fanyi-api.baidu.com/manage/developer> and register as an individual developer
2. enable `通用文本翻译API` service
3. get `appid` and `key` (密钥) 
4. set `appid` and `key` with `source = "baidu"` using `set_translate_option()`
...
3. create a translation service with free tier pricing version (you need a visa/master card to complete registration and will not be charged until you exceed 2 million characters monthly)
...
6. (bonus) you can also create `术语表` (dictionary for the terms) as a user-defined dictionary and get the dict id to help you get precise translation in a certain domain.
...
7. set `appid` and `key` with `source = "volcengine"` using `set_translate_option()` and
...
3.  set the API key as `key` (no need for setting `appid`) with `source = "chatglm"` using `set_translate_option()`
4.  have fun with `chatglm_translate()`
5.  you can also set the model you prefer to use with the option `user_model` when using `set_translate_option()`,
...
2.  login with your account, go to <https://platform.deepseek.com/api_keys> to get your API key;
3.  set the API key as `key` (no need for setting `appid`) with `source = "dsk"` using `set_translate_option()`
4.  have fun with `dsk_translate()`
5.  you can also set the model you prefer to use with the option `user_model` when using `set_translate_option()`,
    currently `user_model` can be set as `deepseek-chat` (by default), `deepseek-coder`.
